<div class="span3">
    <div class="well sidebar-nav">
        <ul class="nav nav-list">
            ...
            <li><?php echo $this->Html->link(__('New Image'), array('action' => 'add_home')); ?></li>
            <li class="divider"></li>              
...
<div class="span9">
    ...
    <div class="page-header">
        <h1><?php echo __('Images') . ' de tipo Home'; ?></h1>
    </div>
    ...
    <table class="table table-bordered table-striped">
        <thead>
            <tr>
                ...
                <th style="text-align: center;"><?php echo $this->Paginator->sort('link', 'Tiene Link'); ?></th>
                <th><?php echo ' Link'; ?></th>
                <th><?php echo $this->Paginator->sort('title'); ?></th>
                <th><?php echo $this->Paginator->sort('alt', 'Texto Alternativo'); ?></th>
        ...
        <?php foreach ($images as $image): ?>
            <tr>
                ...
                <?php
                    echo (!empty($image['Image']['link']) || !empty($image['Image']['referenced_type'])) ? '<span class="label label-success"><i class="icon icon-white icon-ok"></i></span> ' : '<span class="label label-important"><i class="icon icon-white icon-remove"></i></span> ';     
                ?>
                </td>
                <td>
                <?php
                    if (!empty($image['Image']['link'])):
                        echo '<span class="label"><i class="icon-white icon-globe"></i></span> <a  target="_blank" href="http://' . $image['Image']['link'] . '">' . $image['Image']['link'] . '</a>';
                    else:
                ...
                <td style="text-align: center;">
                    ...
                    <?php echo $this->Html->link('<span class="icon-pencil"></span>', array('action' => 'edit_home', $image['Image']['id']), array('class' => 'btn btn-small', 'escape' => false)); ?>
                    
                    <?php echo $this->Form->postLink('<span class="icon-white icon-trash"></span>', array('action' => 'delete_home', $image['Image']['id']), array('class' => 'btn btn-danger btn-small', 'escape' => false), __('Are you sure you want to delete # %s?', $image['Image']['id'])); ?>
                    </div>
                </td>
            </tr>
        <?php endforeach; ?>        
